...
namespace Catch
{
    ...
    namespace
    {
        ...
        {
        public:
            explicit TestGroup(IStreamingReporterPtr &&reporter, Config const *config) : m_reporter(reporter.get()),
                                                                                         m_config{config},
...
namespace Catch
{
                                                                  ...
                                                                  lineInfo(_lineInfo)
    {
        ...
        std::sort(begin(tags), end(tags), [](Tag lhs, Tag rhs)
                  { return lhs.lowerCased < rhs.lowerCased; });
        tags.erase(std::unique(begin(tags), end(tags), [](Tag lhs, Tag rhs)
                               { return lhs.lowerCased == rhs.lowerCased; }),
                   end(tags));
    }
    ...
    {
        return m_invoker == rhs.m_invoker && m_info->name == rhs.m_info->name && m_info->className == rhs.m_info->className;
    }
...
namespace Catch
{
    ...
    Section::Section(SectionInfo &&info) : m_info(std::move(info)),
                                           m_sectionIncluded(
    ...
    namespace
    {
        ...
        public:
            ...
            TapAssertionPrinter(std::ostream &_stream, AssertionStats const &_stats, std::size_t _counter)
                : stream(_stream), result(_stats.assertionResult), messages(_stats.infoMessages), itMessage(_stats.infoMessages.begin()), printInfoMessages(true), counter(_counter) {}
